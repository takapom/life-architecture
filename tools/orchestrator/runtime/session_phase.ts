...
import { summarizeRuntimeLifecycle } from "../shared/runtime_lifecycle";
import {
...
import { readSessionArtifactSnapshot } from "../shared/session_state";

type Command = "select-phase" | "run";
type Phase = "execute" | "resume" | "close" | "closed";
...
function usage(): string {
  return [
    "Usage:",
    "  bun session_phase.ts select-phase --repository <owner/repo> [--issue <number|url>] [--parent-issue <number|url>] [--state-backend <github|local>] [--state-dir <path>] [--session-id <value>]",
    "  bun session_phase.ts run --repository <owner/repo> [--issue <number|url>] [--parent-issue <number|url>] [--state-backend <github|local>] [--state-dir <path>] [--task-source <issues.json>] [--skills-config <path>] [--profile <name>] [--allow-dirty-base] [--run-issue <number>] [--run-id <value>] [--base-branch <name>]",
    "",
    "Notes:",
    "  - phase selection uses session artifacts first, then GitHub task issue state under the requested issue selector.",
    "  - --issue is the canonical selector for the standard path.",
    "  - resume reuses execute runtime with an existing session state directory.",
    "  - already-closed sessions are detected from the canonical runtime lifecycle and do not re-enter close.",
    "  - if GitHub task state implies close but session artifacts are missing, run fails closed.",
    "  - parent issues remain tracking-only; grouped work is an explicit exception surface.",
  ].join("\n");
}
...
function resolveRuntimeEnv(): NodeJS.ProcessEnv {
  // biome-ignore lint/style/noProcessEnv: orchestration runtime must read the live process env at the CLI boundary for delegated phase execution.
  return process.env;
}
...
export function selectPhaseFromSessionState(
  ...
  const snapshot = readSessionArtifactSnapshot(input.stateDir);
  ...
  if (runtime.status === "closed") {
    return {
      phase: "closed",
      source: "session",
      reason: "session closeout artifacts already exist",
  ...
  if (!snapshot.state) {
    if (snapshot.has_session_artifacts) {
      fail(`session artifacts exist but state.json is missing: ${input.stateDir}`);
    }
    return null;
  }

  const nodeStatuses = Object.values(snapshot.state.nodes)
    .map((entry) => {
  ...
  if (nodeStatuses.length === 0) {
    fail(`session state contains no nodes: ${snapshot.state.source_path}`);
  }

  const activeStatuses = nodeStatuses.filter((status) => !TERMINAL_NODE_STATUSES.has(status));
  if (activeStatuses.length > 0) {
    const lockOwner = runtime.lock?.owner_label || "";
    const interruptedCommand =
      runtime.last_command && runtime.last_command.stage === "started" && !runtime.lock
        ? runtime.last_command.command
        : "";
    let reason = `session has ${activeStatuses.length} non-terminal node(s)`;
    if (lockOwner) {
      reason = `session is locked by ${lockOwner}`;
    } else if (interruptedCommand) {
      reason = `session requires recovery after interrupted ${interruptedCommand}`;
    }
    return {
      phase: "resume",
      source: "session",
...
export function selectPhaseFromTaskIssues(input: TaskIssueSelectionInput): SelectionResult {
  ...
  if (parentTasks.length === 0) {
    fail(
      `parent issue #${input.parentIssueNumber} has no linked task issues; grouped execution cannot start without canonical child tasks`
    );
  }
...
async function selectPhase(repoRoot: string, cli: Cli): Promise<SelectionResult> {
  ...
  const issueGraph = await loadTaskIssues({ repository, state: "open" });
  const taskSizingCertification = collectTaskSizingCertificationReport(issueGraph.issues);
  if (taskSizingCertification.failing_task_count > 0) {
    fail(
      `task sizing certification failed for open task issues: ${taskSizingCertification.failing_tasks
        .map((entry) => {
...
export function buildPhaseInvocations(
  ...
  const executeRuntime = path.join(runtimeRoot, "execute.ts");
  const closeRuntime = path.join(runtimeRoot, "close.ts");
  ...
  switch (selection.phase) {
    ...
    case "resume": {
      ...
      const issueSelector =
        selection.task_issue_number > 0
          ? String(selection.task_issue_number)
          : String(cli.issue || "").trim();
      pushValueFlag(args, "--issue", issueSelector);
...
async function main(): Promise<void> {
  ...
  const invocations = buildPhaseInvocations(runtimeRoot, cli, selection);
...
if (import.meta.main) {
  await main().catch((error: unknown) => {
    process.stderr.write(`session_phase failed: ${(error as Error).message}\n`);
    process.exit(1);
  });
}
